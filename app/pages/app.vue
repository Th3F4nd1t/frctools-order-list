...
import type { Order, OrderEditorSubmitPayload, OrderEditorValues } from '~/types/orders'
...
const csvExportColumns: CsvColumn[] = [
  ...
  {
    label: 'Status',
    getValue: row => statusLookup[row.status as StatusKey]?.label ?? row.status
  },
...
// Stats / filter state for table view
...
async function createOrderFromEditor(values: OrderEditorValues): Promise<boolean> {
  try {
...
function buildCsvContent(rows: OrderTableRow[]) {
  ...
  const dataLines = rows.map(row =>
    csvExportColumns.map(column => escapeCsvValue(column.getValue(row))).join(',')
  )
